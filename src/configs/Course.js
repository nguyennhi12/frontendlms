...
const apiSettings = {
    getDetailCourses: async (idCourse) => {
      ...
      getAllCourse: async () => {
        const token =localStorage.getItem("accessToken");
        const endpoint = `${API_URL}${Course}/searchHistoryCourse?status=ALL&pageNumber=1&pageSize=1`;
        const result = await( await fetch(endpoint,
            {
                method:'GET',    
                headers:{                                  
     ...
     getOneRoadMap: async (idroadmap) => {
      ...
      createAssignment: async (assignmnet) => {       
        const token =localStorage.getItem("accessToken");     
        //console.log(student.get("Score"),student.get("idStudent"),student.get("idAssignment"),student.get("idCourse"))
        const endpoint = `${API_URL}${CreateAssigments}/createAssigments`;
        const result = await( await fetch(endpoint,
            {
                method:'POST',
                body:assignmnet,  
